from transformers import AutoTokenizer, AutoModelForCausalLM
from launcher.utils.devices.device_manager import DeviceManager
import torch


class AdvancedTextGenerator:
    def __init__(self, model_name='gpt2', device_id='cpu', half_model_accuracy=False):
        self.model = None
        self.tokenizer = None
        self.device_manager = DeviceManager()
        self.set_device(device_id)
        self.load_model(model_name)
        self.model_name = model_name
        self.half_model_accuracy = half_model_accuracy
        self.device_id = device_id

    def set_device(self, device_id):
        self.device_manager.set_device(device_id)
        self.device = self.device_manager.get_device()

    def load_model(self, model_name):
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)
            self.model = AutoModelForCausalLM.from_pretrained(model_name).to(self.device)
            if self.half_model_accuracy:
                self.model = self.model.half()
        except Exception as e:
            print(f"Error loading model: {e}, line: {e.__traceback__.tb_lineno}")

    def generate_text(self, prompt, max_length=50, temperature=1.0, top_k=50, top_p=0.95, repetition_penalty=1.0):
        # Manage large text prompts with context handling
        max_length = min(max_length, 2048)  # Ensuring max_length is within a practical limit

        inputs = self.tokenizer(prompt, return_tensors='pt').to(self.device)
        input_ids = inputs['input_ids']
        attention_mask = inputs.get('attention_mask')

        # Generate text in chunks to handle large contexts
        generated_text = ''
        current_input_ids = input_ids
        while len(generated_text.split()) < max_length:
            outputs = self.model.generate(
                input_ids=current_input_ids,
                attention_mask=attention_mask,
                max_length=min(max_length + len(current_input_ids[0]), 2048),  # Limiting output length
                temperature=temperature,
                top_k=top_k,
                top_p=top_p,
                repetition_penalty=repetition_penalty,
                do_sample=True
            )
            chunk = self.tokenizer.decode(outputs[0], skip_special_tokens=True)
            generated_text += chunk
            current_input_ids = outputs[:,
                                -len(input_ids[0]):]  # Using the last part of the generated output as context

        return generated_text[:max_length]

    def manage_requests(self, prompt, max_length=50, temperature=1.0, top_k=50, top_p=0.95, repetition_penalty=1.0,
                        max_requests=5):
        """
        Manages multiple requests with rate limiting.
        """
        import time
        results = []
        for _ in range(max_requests):
            result = self.generate_text(prompt, max_length, temperature, top_k, top_p, repetition_penalty)
            results.append(result)
            time.sleep(1)  # Throttle requests to avoid overload
        return results


if __name__ == '__main__':
    # Initialize device manager and set device
    dm = DeviceManager()
    dm.print_available_devices()

    # Example device choice
    device_id = 'cpu'  # or 'cpu', 'cuda:1', etc.

    generator = AdvancedTextGenerator(model_name='gpt2-large', device_id=device_id)
    prompt = "What is quasar?"

    # Generate text with multiple requests
    generated_texts = generator.manage_requests(
        prompt,
        max_length=100,  # Adjust as needed
        temperature=0.7,
        top_k=50,
        top_p=0.9,
        repetition_penalty=1.2
    )

    print(f"\nFinal Generated Texts:\n{generated_texts}")
